...
class ReporteService:
    ...
    async def crear_reporte(self, payload: ReporteCreate) -> ReportePublic:
        ...
        if not usuario_id:
            nuevo_user = UsuarioInternal(nombre="Ciudadano Anónimo", telefono="0000000000")
            user = self.usuario_repo.create(nuevo_user.model_dump())
            usuario_id = user.id
        ...
        reportes_previos = [
            {"id": r.id, "ubicacion_lat": r.ubicacion_lat, "ubicacion_lng": r.ubicacion_lng, "tipo_emergencia": r.tipo_emergencia, "grupo_incidente_id": r.grupo_incidente_id}
        ...
        logger.info(f"Reporte creado exitosamente ID={reporte_db.id} Severidad={severidad_enum.value}")
